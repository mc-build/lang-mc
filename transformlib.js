...
module.exports = context => {
	...
	const json_files = Object.keys(files)
	...
	const func_from_file = Object.fromEntries(
		Array.from(Object.entries(file_from_func)).map(([key, value]) => [value, key])
	)
	...
	function flatten(entries) {
		...
		while (unseen.size > 0) {
			...
			if (!res.has(item)) {
				const children = dependencies[file_from_func[item]]
